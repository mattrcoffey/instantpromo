...
?>
<?php
class MagenThemes_MTBelano_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getStoreConfig('mtbelano/' . $optionString);
    }
    ...
    {
        if ($storeId)
            return Mage::getStoreConfig('mtbelano_design', $storeId);
        else
            return Mage::getStoreConfig('mtbelano_design');
    }
    ...
    {
        return Mage::getStoreConfig('mtbelano_design/' . $optionString, $storeCode);
    }
    ...
    {
        return Mage::getStoreConfig('mtbelano_layout/' . $optionString, $storeCode);
    }
     ...
     public function getLabel(Mage_Catalog_Model_Product $product)
    {
        ...
        if ( $this->getCfg("product_labels/new") && $this->_checkNew($product) ) {
            $html .= '<div class="product-'.$this->getCfg("product_labels/new").'-label">'.$this->__('New').'</div>';
        }
        if ( $this->getCfg("product_labels/sale") && $this->_checkSale($product) ) {
            $html .= '<div class="product-'.$this->getCfg("product_labels/sale").'-label">'.$this->__('Sale').'</div>';
        }
	 ...
	 public function getAltImgHtml($product, $w, $h, $imgVersion='small_image')
    {
        ...
        if ($gal = $product->getMediaGalleryImages())
        {
            ...
            {
                return
                    '<img class="img-responsive alt-img lazy" data-src="' . Mage::helper('mtbelano/image')->getImg($product, $w, $h, $imgVersion, $altImg->getFile()) . '" alt="' . $product->getName() . '" />';
            }
        }
